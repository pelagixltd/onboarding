# EasySOC — Tenant Data Sovereignty & Access v6
...
> **Supersedes:** Tenant Data Sovereignty & Access v5 — two factual corrections, no access-model changes: (1) §2's Sentinel incidents row said the agent writes comments and resolution classification via **ARM PATCH** — the actual code (`sentinelbackend.go`'s `postComment`/`putIncidentProperties`) uses ARM **PUT** throughout; Sentinel incidents don't support PATCH at all. Corrected. (2) §3/§5's container-registry references still named the retired `easysoc.azurecr.io` (confirmed dead via `az acr show -n easysoc` → `ResourceNotFound`); corrected to the current `easysoccr-gyfwc2acakhmg5h0.azurecr.io`.
