...
| `-AzureOpenAiApi` | Used when `-LlmBackend azure_openai`. Blank (default) = Chat Completions, right for `gpt-5.4` and `gpt-5.6-terra`. Set `responses` for `gpt-6-sol`: that model refuses tools with reasoning on Chat Completions. Prompted if omitted; written to `easysoc-deploy.config.ps1` as `$AzureOpenAiApi` and passed to the container as `AZURE_OPENAI_API` |
...
| `400 Bad Request ... Function tools with reasoning_effort are not supported for gpt-6-sol in /v1/chat/completions` in agent logs; every incident goes to review | The deployment is `gpt-6-sol` but `$AzureOpenAiApi` is blank, so the agent calls Chat Completions | Set `$AzureOpenAiApi = "responses"` in `easysoc-deploy.config.ps1` and re-run `deploy-aci.ps1` |
